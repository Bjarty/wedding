...
export default function HoneymoonTracker() {
  const content = siteContent.honeymoon;
  ...
  return (
    <section id={content.sectionId} className="py-32 bg-cream overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-8">
          <div>
            <span className="text-gold font-bold uppercase tracking-[0.3em] text-xs mb-4 block">{content.eyebrow}</span>
            <h2 className="text-7xl font-serif leading-tight">{content.headingLineOne} <br /><span className="italic">{content.headingLineTwo}</span></h2>
          </div>
          <div className="max-w-md">
            <p className="text-taupe italic font-accent text-2xl mb-4">{content.quote}</p>
            <p className="text-sm uppercase tracking-widest font-bold opacity-60">{content.startDate}</p>
          </div>
        </div>
        ...
        <div ref={containerRef} className="relative">
          ...
          <div className="space-y-32">
            {content.stops.map((stop, index) => {
              const Icon = honeymoonIcons[stop.icon];
                ...
                >
                  <div className="w-full md:w-1/2 relative group">
                    <div className="aspect-[4/3] overflow-hidden rounded-3xl shadow-2xl">
                      <img src={stop.image} alt={stop.location} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
                    </div>
                  ...
                  <div className="w-full md:w-1/2 text-center md:text-left">
                    ...
                    <p className="font-accent italic text-2xl text-taupe mb-4">{stop.location}</p>
