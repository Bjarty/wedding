...
export default function RSVPForm() {
  ...
  if (status === 'success') {
    return (
      <section id="rsvp" className="py-32 bg-cream text-center">
        <motion.div
        ...
        >
          ...
          <p className="font-accent italic text-xl text-taupe leading-relaxed">
            Your response has been received with love. We can't wait to celebrate this journey with you.
          </p>
          <button 
  ...
  return (
    <section id="rsvp" className="py-32 bg-stone-dark relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-7xl font-serif text-cream mb-4">Will You <span className="text-gold italic">Join Us?</span></h2>
          <p className="text-cream/60 uppercase tracking-[0.3em] font-bold text-sm">Please RSVP by July 1st, 2026</p>
        </div>
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest font-bold text-taupe ml-1">Your Full Name</label>
              <input 
                required
                type="text" 
                value={formData.name}
            ...
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-widest font-bold text-taupe ml-1">Email Address</label>
              <input 
                required
                type="email" 
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <label className="text-xs uppercase tracking-widest font-bold text-taupe ml-1">Attendance</label>
              <div className="flex gap-4">
                <button
                  type="button"
                  onClick={() => setFormData({...formData, attending: 'yes'})}
                  className={`flex-1 py-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${formData.attending === 'yes' ? 'border-gold bg-gold/5 text-gold' : 'border-stone-dark/5 bg-white text-taupe'}`}
                >
                  <Wine className="w-5 h-5" />
                  <span className="text-xs font-bold uppercase tracking-widest">Joyfully Attend</span>
                </button>
                <button
                  type="button"
                  onClick={() => setFormData({...formData, attending: 'no'})}
                  className={`flex-1 py-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${formData.attending === 'no' ? 'border-gold bg-gold/5 text-gold' : 'border-stone-dark/5 bg-white text-taupe'}`}
                >
                  <Wine className="w-5 h-5 opacity-40 rotate-180" />
                  <span className="text-xs font-bold uppercase tracking-widest">Regretfully Decline</span>
                </button>
              </div>
            </div>
            <div className="space-y-4">
              <label className="text-xs uppercase tracking-widest font-bold text-taupe ml-1">Total Guests</label>
          ...
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-widest font-bold text-taupe ml-1">Dietary Requirements or Wishes</label>
            <textarea 
